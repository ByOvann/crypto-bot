"""
===========================================
  Telegram Crypto Bot — BTC + Leaderboard
...
def format_leaderboard(filepath: str, judul: str, periode: str) -> str:
    ...
    for i, (uid, info) in enumerate(top5):
        ...
        rows += f"{medals[i]} {username} — `{info['pesan']} pesan`\n"
...
# ──────────────── HANDLE SEMUA PESAN (tracker + keyword) ───────────────────
...
async def handle_pesan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if teks == "btc":
        ...
        await update.message.reply_text(
            format_leaderboard(FILE_HARIAN, "🏆 *Leaderboard Harian*", periode),
            parse_mode="Markdown"
        )
...
async def cmd_start(update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• btc atau /btc — Harga BTC sekarang\n"
        "• fg atau /fg — Fear & Greed Index\n"
        "• top atau /top — Leaderboard hari ini\n"
        "• topweek atau /topweek — Rekap minggu ini\n"
        "• topmonth atau /topmonth — Rekap bulan ini\n"
...
async def main():
    ...
    scheduler.add_job(kirim_leaderboard_harian,  trigger="cron", hour=13, minute=0, args=[app.bot])
    scheduler.add_job(kirim_leaderboard_mingguan, trigger="cron", day_of_week="mon", hour=0, minute=0, args=[app.bot])
    scheduler.add_job(kirim_leaderboard_bulanan,  trigger="cron", day=1, hour=0, minute=0, args=[app.bot])
